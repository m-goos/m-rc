---
title: Jenkins and Docker - Automated Seed Job Setup - part 2
date: 2021-09-29
description: Setting up jobs from templates in a repository (JCasC & JobDSL plugin)
---
...
Now finally run the Jenkins container, refer to [Blog 1](https://m-rc.nl/blog/jenkins-docker-configuration-as-code-setup-part-1/) in this series if you are looking for an explanation for the flags/options I'm using here:
...
- admin: `admin`
- password: `password`
- _to stop this container when you are done: `docker stop [CONTAINER-ID]`_
...
![jenkins-seeded](/blog/jenkins-docker-job-setup-automation-part-2/jenkins-seeded.png)
...
![seed-1-ran](/blog/jenkins-docker-job-setup-automation-part-2/seed2.png)

## 5. Why the initial job needs to be started manually

Ideally, the seed job would be run once the controller is done starting up. That seemed simple enough, using the jobDSL queue command. The command can `queue` the seed job to be run once the controller is in its _ready_ state, added below the declaration of our initial job:
