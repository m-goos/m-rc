---
title: Build and deploy a Markdown blog with Angular
date: 2021-02-16
description: A sleek, minimal blog styled with bootstrap and hosted on AWS S3
---

# How to: build a blog with Angular 11, Markdown, Bootstrap 5 and the AWS Cloud (S3)

> 🚀 Showcase: [deployed version](https://d1c4tebsr2ti4f.cloudfront.net)

Set up your own blogging website from scratch, within an Angular 11 project. My goal is to show you how the following technologies work together: Angular 11, Bootstrap 5, AWS S3 (cloud storage) and Markdown. This post guides you in adding the simple blogging functionality with a very clean look.

Together with you I will set up the blogging module for your website, style it and host it in the AWS cloud, in a so-called _S3 bucket_ for static website hosting. The blogs will be written in [Markdown](https://daringfireball.net/projects/markdown/), a highly portable markup format, that I use often. With this setup, I can post my writings across platforms with minimal editing. And that is just great!

If one of these technologies might be new to you:

- Bootstrap is a markup framework for building responsive websites. Where Bootstrap 4 still used jQuery, Bootstrap 5 (_beta_) now uses JavaScript.
...
|![List of blog posts](/blog/build-and-deploy-angular-markdown-blog-aws-s3/example.png)|![View of one post](/blog/build-and-deploy-angular-markdown-blog-aws-s3/example-1.png)|
|:-:|:-:|
...
4. Clean styling: css-template for Markdown and Bootstrap 5
5. Host the blog using cloud storage: AWS S3 bucket
6. Looking ahead: possible improvements such as a CDN and CI/CD

### Prerequisites

- A Full Stack JavaScript Development environment; see my [setup guide](https://TODO) for MacOS
- Some experience with Angular; see my [Angular starter guide](https://TODO)
...
Not on version 11 yet? Update: `npm update -g @angular/cli@latest`. This should take you to the latest version of Angular. Now keep in mind that _updating might have consequences_ for the compatibility of the CLI with your current Angular projects. See the Angular Update Guide ([x](https://update.angular.io/?v=10.0-11.0)) for detailed information about updating your project. By adding `@latest`, you are updating to the latest version; generally, Angular has a major release every 6 months.
...
| ![Project structure with blog components](/blog/build-and-deploy-angular-markdown-blog-aws-s3/project-structure-1.png) | ![Project structure with blog components](/blog/build-and-deploy-angular-markdown-blog-aws-s3/project-structure-2.png) |
| :--------------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------------------------------------: |
|                                               _Project structure after_                                                |                                              _generating blog components_                                              |
...
| ![Blog Overview](/blog/build-and-deploy-angular-markdown-blog-aws-s3/intermediate-result-list.png) | ![Individual post](/blog/build-and-deploy-angular-markdown-blog-aws-s3/intermediate-result-post.png) |
| :------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------------------: |
|                                       Overview of blog posts                                       |                                           Individual post                                            |

Alright, let's build the blog, following the three steps outlined above.

#### **1. Configuring blog.module.ts** - _[view this code on Github](https://github.com/m-goos/angular-blog/blob/main/angular-markdown-blog/src/app/blog/blog.module.ts)_
...
#### **2. Setting up the list of blog entries** - _[view this code on Github](https://github.com/m-goos/angular-blog/tree/main/angular-markdown-blog/src/app/blog/entry-list)_
...
#### **3. Rendering the post we want to see** - _[view this code on Github](https://github.com/m-goos/angular-blog/tree/main/angular-markdown-blog/src/app/blog/post)_
...
so it should get the blog at `/assets/how-i-got-up-to-speed-in-angular/how-i-got-up-to-speed-in-angular.md`. Every blog folder also holds its own `images` folder - and this makes the blogs very portable. With this setup, I can deploy my blogs on different platforms with _minimal_ editing. And that is just great!

| ![Assets folder](/blog/build-and-deploy-angular-markdown-blog-aws-s3/assets-folder.png) |
| :-------------------------------------------------------------------------------------: |
|                             _Assets folder in the project_                              |
...
|                ![Splendor theme applied](/blog/build-and-deploy-angular-markdown-blog-aws-s3/splendor-result.png)                 |
| :-------------------------------------------------------------------------------------------------------------------------------: |
| _Applying the [Splendor](https://github.com/Markdowncss/splendor) theme to our Markdown blog - navbar still needs some attention_ |
...
| ![Target the markdown-post id](/blog/build-and-deploy-angular-markdown-blog-aws-s3/wrapping-scss.png) |
| :---------------------------------------------------------------------------------------------------: |
|                                      Target the markdown-post id                                      |

As you see, both the navbar and our list of blog posts still need a bit of attention:
![Navbar and blog post list need attention](/blog/build-and-deploy-angular-markdown-blog-aws-s3/intermediate-result-list.png)
...
| ![List of blog posts](/blog/build-and-deploy-angular-markdown-blog-aws-s3/example.png) | ![View of one post](/blog/build-and-deploy-angular-markdown-blog-aws-s3/example-1.png) |
| :------------------------------------------------------------------------------------: | :------------------------------------------------------------------------------------: |
|                                  _List of blog posts_                                  |                                _View of a single post_                                 |
